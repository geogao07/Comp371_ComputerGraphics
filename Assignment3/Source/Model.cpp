...
: mName("UNNAMED"), mPosition(0.0f, 0.0f, 0.0f), mScaling(1.0f, 1.0f, 1.0f), mRotationAxis(0.0f, 1.0f, 0.0f),
  mRotationAngleInDegrees(0.0f), mMaterialCoef(0.0f, 0.0f, 0.0f),n(0.0f),mAnimation(nullptr)
{
}
...
bool Model::ParseLine(const std::vector<ci_string> &token)
{
    if (token.empty() == false)
	{
		...
		else if (token[0] == "material")
		{
			assert(token.size() > 4);
			assert(token[1] == "=");
			mMaterialCoef.x= static_cast<float>(atof(token[2].c_str()));
			mMaterialCoef.y= static_cast<float>(atof(token[3].c_str()));
			mMaterialCoef.z = static_cast<float>(atof(token[4].c_str()));
