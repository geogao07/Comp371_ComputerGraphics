...
//    float randomValue = EventManager::GetRandomFloat(0.0f, averageTimeBetweenEmission);
    ...
    if (timeSinceLastParticleEmitted > averageTimeBetweenEmission && mInactiveParticles.size() > 0)
    {
		...
		vec3 velocityVectorNormal = normalize(mpDescriptor->velocity);
		
		vec3 normalLine = cross(velocityVectorNormal, vec3(EventManager::GetRandomFloat(0, 1.0), EventManager::GetRandomFloat(0, 1.0), EventManager::GetRandomFloat(0, 1.0)));	
			
		vec3 normalLineNormal = normalize(normalLine);

		//1st rotation
		float fitstAngle = EventManager::GetRandomFloat(0, mpDescriptor->velocityAngleRandomness);	//calculate first angle
		glm::mat4 firstRotate = glm::mat4_cast(angleAxis(radians(fitstAngle), normalLineNormal));	//convert quat to mat
		vec3 verticalvec = vec4(newParticle->velocity, 0.0f)*firstRotate;	// vector rotates along perpencdicular axis
																						
		float secondAngle = EventManager::GetRandomFloat(0, 360);	//calculate second random angle
		glm::mat4 secondRotate = glm::mat4_cast(angleAxis(radians(secondAngle), velocityVectorNormal));	//rotates cone shape
		newParticle->velocity = vec4(verticalvec, 0.0f)*secondRotate; //obtain new vec
    ...
    for (std::list<Particle*>::iterator it = mParticleList.begin(); it != mParticleList.end(); it++)
    {
		...
		else if (p->currentTime >mpDescriptor->fadeInTime&&p->currentTime<p->lifeTime - mpDescriptor->fadeOutTime)
		{
			...
			(p->currentTime > (p->lifeTime - mpDescriptor->fadeOutTime)&&p->currentTime<p->lifeTime)
		{
